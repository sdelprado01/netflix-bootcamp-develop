...
public class ChapterServiceImpl implements ChapterService {
	...
	@Override
	public List<ChapterRequestRest> getChaptersByTvShowIdAndSeasonNumber(Long tvShowId, short seasonNumber)
			throws NetflixException {
		return chapterRepository.findBySeasonTvShowIdAndSeasonNumber(tvShowId, seasonNumber).stream()
				.map(chapter -> modelMapper.map(chapter, ChapterRequestRest.class)).collect(Collectors.toList());
	}

	@Override
	public ChapterRequestRest getChapterByTvShowIdAndSeasonNumberAndChapterNumber(Long tvShowId, short seasonNumber,
		...
		return modelMapper.map(chapter, ChapterRequestRest.class);
	}
